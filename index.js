const submitFrom = document.querySelector(".search-form");
const searchBar = document.querySelector(".search-bar");
const content = document.querySelector(".content");

submitFrom.addEventListener("submit", (e) => {
  e.preventDefault();
  const formData = new FormData(submitFrom);
  renderSearchedMovie(formData.get("movie-name"));
});
...
async function renderRandomMovies() {
  ...
  const randomImdbIds = [];
  while (randomImdbIds.length < imdbIds.length) {
    const randomId = imdbIds[Math.floor(Math.random() * imdbIds.length)];
    if (!randomImdbIds.includes(randomId)) {
      randomImdbIds.push(randomId);
    }
  }
  const htmlArray = await Promise.all(
    randomImdbIds.map(async (id) => {
...
async function renderSearchedMovie(name) {
  const data = await fetchApiTitle(name);
    ...
    content.innerHTML = `<div class="movie-box">
                ...
                <div class="movie-desc">
                  ...
                  <div class="movie-info">
                    <span class="duration">${data.Runtime}</span>
                    <span class="genre">${data.Genre}</span>
                    <button class="wishlist-btn" id=${data.imdbID}>
                      <img src="./assests/plus-icon.png" alt="add-icon" />
                      Watchlist
                    </button>
                  </div>
                  <p class="movie-brief">
...
function checkMovieAdded(id) {
  ...
  if (data.includes(id)) {
    ...
    return `<button class="wishlist-btn" id=${id}>
    <img src="./assests/plus-icon.png" alt="add-icon" />
    Watchlist
    </button>`;
  }
  ...
  document.querySelector(".content").addEventListener("click", (e) => {
    ...
    if (movieId) {
      ...
      if (btn.classList.contains("added")) {
        btn.innerHTML = `<img src="./assests/plus-icon.png" alt="add-icon" />
        Watchlist`;
        ...
        btn.innerHTML = `<img src="./assests/tick-icon.png" alt="add-icon" />
        Added to Watchlist`
        data.push(movieId);
